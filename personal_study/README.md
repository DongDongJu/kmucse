...
* 아이팝콘 대충 정리하고 워크샵 정리했음

* 이제 docker 에 대해서 좀 살펴보기 시작
...
도커에서 이 파일 시스템을 활용하고 있음

읽기전용으로만 사용되고 사용자가 안에서 추가한 것들은 write 브랜치에 저장함.

거기다가 커밋하고 더하면 새로운 버전의 read only 이미지가 생기는거임

결과적으로 기본적인 데이터를 이미지로 두고 변화하는 데이터의 정보만 저장하는 파일시스템
~~~

리눅스 파일시스템에는 bootfs 랑 rootfs가 있는데, bootfs(커널 포함) 이 보통 최하단 레이어로 자리잡는다.

그위에 os 별 다른 image 들을 하나씩 레이어로써 올리게 되는 구조.


~~~
## LXC(LinuX COntainers)

내가 알기론 cgroups 베이스인듯 맞나?
